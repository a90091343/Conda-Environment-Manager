...
translation_dict = {
    ...
    "(4) 请输入此环境注册的Jupyter内核的显示名称(为空使用默认值):": "(4) Please enter the display name of the Jupyter kernel registered for this environment (leave blank for default):",
    ...
    "(3.{idx}) 请输入环境{LIGHT_CYAN(env_name)}注册的Jupyter内核的显示名称(为空使用默认值):": "(3.{idx}) Please enter the display name of the Jupyter kernel registered for the environment {LIGHT_CYAN(env_name)} (leave blank for default):",
    ...
    "(4) 请输入注册的Jupyter内核的显示名称(为空使用默认值):": "(4) Please enter the display name of the registered Jupyter kernel (leave blank for default):",
    "(3.{idx}) 请输入环境{LIGHT_CYAN(env_name)}复制后的环境名称(为空使用默认值):": "(3.{idx}) Please enter the name of the environment after copying the environment {LIGHT_CYAN(env_name)} (leave blank for default):",
    ...
    "对于{LIGHT_CYAN(inp)},共找到{LIGHT_CYAN(len(pkginfos_list_raw))}个相关包,搜索结果如上": "Found {LIGHT_CYAN(len(pkginfos_list_raw))} relevant packages for {LIGHT_CYAN(inp)}.",
