...
translation_dict = {
    ...
    "[错误] 无法读取快捷方式文件：": "[Error] Unable to read the shortcut file:",
    ...
    "(1) 请输入想要{BOLD(RED('删除'))}的环境的编号（或all=全部），多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(RED('delete'))} (or all for all envs), separated by spaces:",
    "(2) 确认删除以上环境吗？": "(2) Are you sure you want to delete the above environment(s)? ",
    ...
    "(1) 请输入想要{BOLD(LIGHT_GREEN('新建'))}的环境的名称：": "(1) Please enter the name of the environment you want to {BOLD(LIGHT_GREEN('create'))}: ",
    "新环境名称": "New environment name",
    "{LIGHT_RED('已存在或不符合规范')}，请重新为 {LIGHT_CYAN(name)} 命名：": "{LIGHT_RED('already exists or does not meet the specifications')}, please rename {LIGHT_CYAN(name)}: ",
    "{LIGHT_RED('已存在或不符合规范')}，请重新为 {LIGHT_CYAN(name)} 重命名：": "{LIGHT_RED('already exists or does not meet the specifications')}, please rename {LIGHT_CYAN(name)}: ",
    "{LIGHT_RED('已存在或不符合规范')}，请重新输入：": "{LIGHT_RED('already exists or does not meet the specifications')}, please re-enter: ",
    "(2) 请指定 Python 版本（为空默认最新版）：": "(2) Please specify the Python version (leave blank for the latest version): ",
    "[提示]": "[Tip]",
    " 常用第三方源有：": " Common third-party sources include:",
    "(3a) {LIGHT_RED('安装失败！')}是否启用更多的源重新安装？": "(3a) {LIGHT_RED('Installation failed!')} Do you want to enable more sources to reinstall?",
    ...
    "(1) 请输入想要{BOLD(LIGHT_BLUE('重命名'))}的环境的编号，多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(LIGHT_BLUE('rename'))}, separated by spaces: ",
    "(2) 确认重命名以上环境吗？": "(2) Are you sure you want to rename the above environment(s)? ",
    ...
    "(1) 请输入想要{BOLD(LIGHT_CYAN('复制'))}的环境的编号，多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(LIGHT_CYAN('copy'))}, separated by spaces: ",
    "(2) 确认复制以上环境吗？": "(2) Are you sure you want to copy the above environment(s)? ",
    ...
    " (已失效)": " (Expired)",
    "(0a) 确认清理以上失效项吗？": "(0a) Are you sure you want to clear the above Expired registrations? ",
    ...
    "(1) 确认清空所有 Conda/pip 缓存吗？": "(1) Are you sure you want to clear all Conda/pip caches? ",
    ...
    "(1) 请输入想要{BOLD(GREEN('更新'))}的环境的编号（或all=全部），多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(GREEN('update'))} (or all for all envs), separated by spaces: ",
    "(2) 确认更新以上环境吗？": "(2) Are you sure you want to update the above environment(s)? ",
    "[提示] 已自动启用附加源：": "[Tip] Additional sources have been automatically enabled: ",
    "{LIGHT_RED('[警告]')} 检测到如下由 Pip 安装的包，将不会被更新：": "{LIGHT_RED('[Warning]')} The following packages installed by Pip will NOT be updated:",
    "[提示] 检测到如下已固定版本的 Conda 包，将不会被更新：": "[Tip] The following Conda packages with pinned versions are detected and will NOT be updated:",
    "(i) 是否为该环境 添加(A){'/删除(D)' if pinned_pkgs else ''} Conda 包的固定版本信息？": "(i) Do you want to add(A){'/delete(D)' if pinned_pkgs else ''} pinned version information for Conda packages in this environment?",
    "(i) 请输入要删除固定版本信息的 Conda 包的编号，多个以空格隔开：": "(i) Please enter the number(s) of Conda packages you want to delete the pinned version information, separated by spaces: ",
    "[提示] 已删除指定 Conda 包的版本固定信息！": "[Tip] The pinned version information of the specified Conda package has been deleted!",
    "(i) 请输入要固定于当前版本的 Conda 包的编号，多个以空格隔开：": "(i) Please enter the number(s) of Conda packages you want to pin to the current version, separated by spaces:",
    '[提示] 默认仅固定主次版本号 (X.Y.*)，若需固定完整版本，请在对应序号前加"{LIGHT_CYAN("=")}"号': '[Tip] By default, only the major and minor version numbers (X.Y.*) are pinned. If you need to fix the complete version, add "{LIGHT_CYAN("=")}" before the corresponding number',
    "当前环境的所有 Conda 包": "Current Conda packages",
    "[提示] 已添加指定 Conda 包的版本固定信息！": "[Tip] The pinned version information of the specified Conda package has been added!",
    "(i) 更新环境 {LIGHT_CYAN(name)} 可能不再保证正确的包依赖关系，是否继续？": "(i) Updating the environment {LIGHT_CYAN(name)} may no longer guarantee correct package dependencies, do you want to continue?",
    ...
    "(1) 请输入要{BOLD(LIGHT_YELLOW('搜索'))}的包关联的 Python 版本（为空默认全版本）：": "(1) Please enter the Python version associated with the package you want to {BOLD(LIGHT_YELLOW('search'))} (leave blank for all versions): ",
    ...
    "(2) 请输入要搜索的包名（支持通配符 *；适用于 Python {target_py_version}）：": "(2) Please enter the package name to search (supports wildcard *; applicable to Python {target_py_version}): ",
    "(2) 请输入要搜索的包名（支持通配符 *；适用所有 Python 版本）：": "(2) Please enter the package name to search (supports wildcard *; applicable to all Python versions): ",
    ...
    "(i) 是否继续为 Python {target_py_version} 查找包? ": "(i) Do you want to continue searching for packages for Python {target_py_version}? ",
    "(i) 是否继续为所有 Python 版本查找包? ": "(i) Do you want to continue searching for packages for all Python versions? ",
    "(1) 请输入想要{BOLD(LIGHT_GREEN('检查完整性'))}的环境的编号（默认为全部），多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(LIGHT_GREEN('check the integrity'))} (default is all), separated by spaces: ",
    "[警告] conda doctor 子命令需要 23.5 及以上版本支持，请在 base 环境升级后重试！": "[Warning] The conda doctor subcommand requires version 23.5 or above, please upgrade the base environment and try again!",
    ...
    "输入错误达到最大次数 ({max_errors})，程序退出。": "The maximum number of input errors ({max_errors}) has been reached, and the program exits.",
    ...
    "(3) 请指定预安装参数（如{LIGHT_YELLOW('spyder')}包等，{LIGHT_GREEN('-c nvidia')}源等，以空格隔开）：": "(3) Please specify the pre-installed parameters (such as the {LIGHT_YELLOW('spyder')} package, {LIGHT_GREEN('-c nvidia')} source, etc., separated by spaces): ",
    ...
    "不全符合[A-Za-z0-9._-]规则": "does NOT fully meet the [A-Za-z0-9._-] rule",
    ...
    "(1) 请输入想要将 Jupyter 内核{BOLD(CYAN('注册'))}到用户的环境编号（或all=全部），多个以空格隔开：": "(1) Please enter the number(s) of environment(s) you want to {BOLD(CYAN('register'))} the Jupyter kernel to the user (or all for all envs), separated by spaces: ",
    "(2) 确认注册以上环境的 Jupyter 内核到用户吗？": "(2) Are you sure you want to register the Jupyter kernel of the above environment to the user? ",
    ...
    "(1) 请输入想要{BOLD(RED('删除'))}的 Jupyter 内核的编号（或all=全部），多个以空格隔开：": "(1) Please enter the number(s) of Jupyter kernel(s) you want to {BOLD(RED('delete'))} (or all for all kernels), separated by spaces: ",
    "[错误] 未检测到有效的 Jupyter 内核编号！": "[Error] No valid Jupyter kernel number detected!",
    "(2) 确认删除以上 Jupyter 内核注册吗？": "(2) Are you sure you want to delete the above Jupyter kernel registration? ",
    "环境 {LIGHT_CYAN(name)} 的历史版本如下：": "The historical versions of the environment {LIGHT_CYAN(name)} are as follows:",
    "(2) 请输入环境 {LIGHT_CYAN(name)} 的历史版本编号 ({LIGHT_YELLOW(f'0-{valid_rev_nums[-1]}')}): ": "(2) Please enter the historical version number of the environment {LIGHT_CYAN(name)} ({LIGHT_YELLOW(f'0-{valid_rev_nums[-1]}')}): ",
    "[{idx}/{len(env_names)}] 正在更新环境 {LIGHT_CYAN(name)} 的所有包...": "[{idx}/{len(env_names)}] Updating all packages of the environment {LIGHT_CYAN(name)}...",
    '(i) 是否为 {LIGHT_CYAN(name)} 启用 "strict-channel-priority" 规则？': '(i) Enable "strict-channel-priority" for {LIGHT_CYAN(name)}?',
    ...
    "(i) 检测到以上无效环境，是否删除？": "(i) Invalid environments are detected above, do you want to delete them? ",
    "[提示] 所有无效环境均已被删除！": "[Tip] All invalid environments have been deleted!",
    "[提示] 加载缓存信息中，请稍等...": "[Tip] Loading cache information, please wait...",
    "(1) 请输入需要查看及回退{BOLD(LIGHT_MAGENTA('历史版本'))}的环境编号：": "(1) Please enter the number of the environment to view and roll back to {BOLD(LIGHT_MAGENTA('historical version'))}: ",
    ...
    "[提示] 已设置 PYTHONNOUSERSITE 环境变量以隔离用户site-packages": "[Tip] The PYTHONNOUSERSITE environment variable has been set to isolate user site-packages",
...
# [设置 6] 在[S]搜索功能时，默认启用的 Channel 源，即 Conda 包的搜索范围""": """# <Hint> These global settings, prefixed with CFG_, control the default behavior of the program and *cannot* be changed during runtime.
...
CFG_CMD_TRIGGERED_PKGS: str = "matplotlib scikit-learn numba pandas ipykernel"
# [Setting 6] The default enabled Channel source during the [S] search function, i.e., the search scope of Conda packages""",
}
